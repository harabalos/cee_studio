...
// LoadingScreen briefly shows its own h1 (just "CEE") on first visit, so
// page.locator("h1") matches 2 elements. Use .last() to grab the page's own
// h1 (which renders below in DOM order).
test("/privacy displays all sections", async ({ page }) => {
  await page.goto("/privacy");
  await expect(page.locator("h1").last()).toContainText(/Privacy Policy|Datenschutz/i);
...
test("/terms shows 3-tier cancellation policy", async ({ page }) => {
  ...
  await expect(page.locator("h1").last()).toContainText(/Terms|Geschäftsbedingungen|Conditions|Termini/i);
...
test("/rules displays 11 house rules sections", async ({ page }) => {
  ...
  await expect(page.locator("h1").last()).toContainText(/Studio Rules|Studio-Regeln|Règlement|Regole/i);
...
test("/impressum shows CEE Studio info", async ({ page }) => {
  ...
  await expect(page.locator("h1").last()).toContainText(/Impressum|Legal Notice|Mentions légales|Note legali/i);
